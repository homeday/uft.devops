...
<html>
	...
	<body leftmargin="0" topmargin="0" marginwidth="0" marginheight="0" style="margin: 0px; padding: 0px; background-color: #e2e2e2;" bgcolor="#e2e2e2">
		<%
			...
			/*
			def getImageBytes(url, userAgent="Groovy Sample Script"){
				return new URL(url).getBytes( useCaches: true, allowUserInteraction: false, requestProperties: ["User-Agent": userAgent])
			}
			
			def getSubBuild(buildName){
				...
				build.getBuilders().each {
				  subBuild -> 
					...
					if (subProject.getName().indexOf(buildName) >= 0) {
					  return subProject.getBuildByNumber(subBuild.getBuildNumber())
					} 
				}
			}*/
			...
			def uftRepViewSetup = "${DVD_Wix_path}\\RunResultsViewer\\MSI\\HP_Run_Results_Viewer.msi"
			def extAccTool = "${DVD_Wix_path}\\Extensibility and Toolkits\\ExtAccTool\\MSI\\HP_Extensibility_Accelerator_Tool.msi"
			...
			<table style="background-color: #FFF" bgcolor="#FFF" class="shadow" width="80%" align="center" cellpadding="5" cellspacing="0">
					...
					<td>
						...
						<table width="20%"  align="left" cellpadding="0" cellspacing="0">
							<tr>
								...
								<td>
									<span class="${build.result.toString()}"><b>${build.result}</b></span>
										<!--<span style="font-size:20px"><b>UFT One</b></span>-->
										<!--<img src="data:image/png;base64, ${image_cartoon.encodeBase64().toString()}" alt="${cartoon_url}" />-->
								</td>
							</tr>
						</table>
					</td>
				...
				<tr>
					<td>
						...
						<br />
						<% if( null != SetupBuild && 
								(build.result == hudson.model.Result.SUCCESS || build.result == hudson.model.Result.UNSTABLE ) ) 
							{ 
						%>
						<table width="100%" align="left" cellpadding="0" cellspacing="0" >
							<tr>
								<td colspan="2">
									<span><b>Installer:</b>&nbsp;
										<a href="${uftSetup}">UFT</a>&nbsp;|&nbsp;
										<a href="${uftSetup}">QC</a>&nbsp;|&nbsp;
										<a href="${uftSetup}">Report View</a>&nbsp;|&nbsp;
										<a href="${uftSetup}">Ext Acc Tool</a>
									</span>
								</td>
							</tr>
						</table>
