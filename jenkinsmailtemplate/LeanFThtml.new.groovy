...
<%      build.getLog(400).each() { line -> 
       if ( line.contains("status : FAILURE") )
        ...
        def jenkinschildjob=line1.split('BUILD_URL_CONSOLE=')
		
		y=y+1%>
		
		 <TR><TD class="console">${jenkinschildjob}</TD></TR>
        <%    }   } %>
		 ...
		 <TR><TD class="console">${line}</TD></TR>
		 <TR><TD class="console">${jobname}</TD></TR>
		 <TR><TD class="console">${consolestring}</TD></TR>
