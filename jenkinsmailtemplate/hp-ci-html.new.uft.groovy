...
<% } %>
  ...
  <%
  if (null != CompilationBuild) {%>
  <TR><TD><b>SVN revisions</b></TD><TD>${CompilationBuild.getEnvironment().LastSuccessfulRevision}-${CompilationBuild.getEnvironment().SVN_REVISION}</TD></TR>
...
<!-- CHANGE SET -->
<TABLE width="100%">
<TR><TD class="bg1" colspan="3"><B>SVN Changes</B></TD></TR>
<% 

def svnfile = null
if (null != CompilationBuild && CompilationBuild.getWorkspace().child("build/info/Build_SVN_Comments.xml").exists()) {
    svnfile = CompilationBuild.getWorkspace().child("build/info/Build_SVN_Comments.xml").readToString() 
} else if ( null != SetupBuild && SetupBuild.getWorkspace().child("build/info/Build_SVN_Comments.xml").exists()){
    svnfile = SetupBuild.getWorkspace().child("build/info/Build_SVN_Comments.xml").readToString() 
}
...
<TD class="bg2"> Date <B> ${it.date}</B></TD>
