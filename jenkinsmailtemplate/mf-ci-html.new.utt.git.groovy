...
%>
  ...
  <TR><TD><b>Parsed&nbsp;Error&nbsp;log</b></TD><TD><A href="${rooturl}${build.url}parsed_console/">${rooturl}${build.url}parsed_console/</A></TD></TR>
...
<% } %>
  ...
  <TR><TD><b>GIT&nbsp;path</b></TD><TD>${build.getEnvironment().GIT_URL}</TD></TR>
  <TR><TD><b>GIT&nbsp;branch</b></TD><TD>${build.getEnvironment().GIT_BRANCH}</TD></TR>
  <TR><TD><b>GIT&nbsp;previous commit</b></TD><TD>${build.getPreviousBuild() ? build.getPreviousBuild().getEnvironment().GIT_COMMIT : ""}</TD></TR>
  <TR><TD><b>GIT latest commit</b></TD><TD>${build.getEnvironment().GIT_COMMIT}</TD></TR>
...
<TD><a target="_blank" href="file:\\\\mydastr01.hpeswlab.net\\products\\${it.attribute('group')}\\${it.attribute('product')}\\${it.attribute('config')}\\${it.attribute('build')}" > ${it.attribute('build')} </a></TD>
...
try {
    ...
    cenv.put(Context.SECURITY_CREDENTIALS, "Na\$hRomMor12");
