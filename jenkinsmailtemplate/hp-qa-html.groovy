...
%>
  ...
  <TR><TD><b>Job&nbsp; URL</b></TD><TD><A href="${rooturl}${build.url}">${rooturl}${build.url}</A></TD></TR>
   <% if  ( build.getEnvironment().Type != null ){ %>
...
<% } %>
  ...
  <TR><TD><b>BuildVersion</b></TD><TD>${build.getEnvironment().BuildVersion}</TD></TR>
  <TR><TD><b>SVN revisions</b></TD><TD>${build.getEnvironment().LastSuccessfulRevision}-${build.getEnvironment().SVN_REVISION}</TD></TR>
   <% if  ( build.getEnvironment().Branch != null ){ %>
  <TR><TD><b>Branch</b></TD><TD>${build.getEnvironment().Branch}</TD></TR>
     <% } %>
  <TR><TD><b>SVN path</b></TD><TD>${build.getEnvironment().SvnBranch}</TD></TR>
...
<% 
if (build.getParent().getWorkspace().child("build/info/Build_SVN_Comments.xml").exists()) {
 %>
 
   <TR><TD class="bg1" colspan="2"><B>SVN Changes</B></TD></TR>
   <% 
    def svnfile = build.getParent().getWorkspace().child("build/info/Build_SVN_Comments.xml").readToString()
...
<TR>
<%
 it.paths.path.each{ path->
  %>
 <TR><TD>${path.@action}</TD><TD> ${path.text()} </TD><TR>
