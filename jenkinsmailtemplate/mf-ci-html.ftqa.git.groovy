...
<%}%>
  ...
  <!-- ROW: Build Type -->
  <TR><TD class="info_label"><b>Build Type</b></TD><TD>${build.getEnvironment().Build_Type} Build</TD></TR>
...
<!-- ROW SECTION: GIT Information -->
  <!-- ROW: GIT Repository -->
<%
def gitRepoFullURL = build.getEnvironment().GIT_Repo_URL
def gitRepoFullName = gitRepoFullURL.split('/')[-2..-1].join('/')
%>
  <TR><TD class="info_label"><b>GIT Repository</b></TD><TD><A herf="${gitRepoFullURL}">${gitRepoFullName}</A></TD></TR>

  <!-- ROW: GIT Branch -->
  <TR><TD class="info_label"><b>GIT Branch</b></TD><TD>${build.getEnvironment().GIT_Branch_Name}</TD></TR>

  <!-- ROW: GIT Commits Range -->
  <TR><TD class="info_label"><b>GIT Commits</b></TD><TD>${build.getEnvironment().GIT_CommitHash_Range}</TD></TR>
<!-- END OF - ROW SECTION: GIT Information -->
...
<%
  if (null != CompilationBuild) {
    ...
    // msbuild log file
    // BUILD/logs/msbuild_UFTAutomation_12.54.1234.9_Build.log
    def msbuildLogFile = "msbuild_UFTAutomation_" + version + "_" + msbuildTarget + ".log"
    //println "*!* ${msbuildLogFile}<br>"
    //if (CompilationBuild.getParent().getWorkspace().child(relDir + msbuildLogFile).exists()) {
      // SAMPLE: \\mydastr01.hpeswlab.net\products\ST\ST\win32_release\12.54.1234.9\BUILD\logs\msbuild_UFTAutomation_12.54.1234.9_Build.log
...
<!-- CHANGE LOGS -->
<%
def commitLogsFile = "commit_logs.xml"
def commitLogsFileContent = null
if (build.getWorkspace().child(commitLogsFile).exists()) {
  commitLogsFileContent = build.getWorkspace().child(commitLogsFile).readToString()
}
if (commitLogsFileContent != null) {
%>
<TABLE width="100%">
  <TR><TD class="bg1" colspan="4"><B>GIT Changelogs</B></TD></TR>
  <TR>
    <TD class="bg7" style="min-width:80px;width:8%">Commit</TD>
...
<% 
  try {
    def commits = new XmlSlurper().parseText(commitLogsFileContent)
    def currentShownCount = 0
    def altBgFlag = true
    def maxShowCountWithMsg = 20
    def commitUrlBase = commits.github?.commit_base_url?.text().trim()
    commits.logentry.each {
      ...
      // check whether exceed the max show count (with message)
      if (currentShownCount == maxShowCountWithMsg + 1) {
        // this is the first one that exceed the max show count
%>

  <TR>
    <TD class="bg8" colspan="4">More than <B>${maxShowCountWithMsg}</B> changelogs are found. The <B>Message</B>s below are abandoned.</TD>
  </TR>

<%
      } // end if [currentShownCount == maxShowCountWithMsg + 1]
%>

  <TR>
<%    if (commitUrlBase) {
%>
    <TD class="${bgClassName}"><PRE><A href="${commitUrlBase}/${it.@hash}">${it.@hash_abbr}</A></PRE></TD>
<%    } else {
%>
    <TD class="${bgClassName}"><PRE>${it.@hash_abbr}</PRE></TD>
<%    }
%>
    <TD class="${bgClassName}"><PRE><A href="mailto:${it.author.@email}">${it.author.text()}</A></PRE></TD>
    <TD class="${bgClassName}"><PRE>${it.date.text()}</PRE></TD>
    <TD class="${bgClassName}"><PRE><B>${it.subject.text()}</B>
${it.body.text()}</PRE></TD>
  </TR>

<%
    } // end commits.logentry.each
  } catch (Exception e) {
%>
...
<%
} // end if [commitLogsFileContent != null]
%>
