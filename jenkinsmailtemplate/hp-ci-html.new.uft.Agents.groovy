...
%>
   <TR><TD><b>UFT Patch Setup</b></TD><TD><A href="${build.getEnvironment().PRODUCTS_STORAGE_WIN}\\FT\\UFTAgents\\${build.getEnvironment().BuildVersion}">Installation Link</A></TD></TR>
<% } 
%>
...
<%
    ...
    for (def line = bufReader.readLine(); line != null; line = bufReader.readLine()) {
      ...
      if ( (line =~ /error / || line =~ /Error / ||line =~ /ERR! /) && !(line =~ /^  /) ) {
        finderrline = true
        lineNumber2 = 1 %>
        <TR><TD class="console">${line}</TD></TR>
      <%}
