...
%>
  ...
  <TR><TD><b>SVN&nbsp;path</b></TD><TD>${build.getEnvironment().SVN_URL}/${build.getEnvironment().SVN_Repo}/${build.getEnvironment().Branch}</TD></TR>
...
if ( build.getEnvironment().LastSuccessfulRevision == build.getEnvironment().SVN_REVISION) { %>
  <TD class="bg2"><B> no changes </B></TD>
<%} else {
  if (null != build && build.getWorkspace().child("build/info/Build_SVN_Comments.xml").exists()) {
    svnfile = build.getWorkspace().child("build/info/Build_SVN_Comments.xml").readToString() 
  }
if ( svnfile != null) {
    try {
