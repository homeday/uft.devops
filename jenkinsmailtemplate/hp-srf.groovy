...
TD.bg6 { color:red; background-color:white; font-size:120% }
...
<TABLE width="100%" cellpadding="0" cellspacing="0">
 ...
 <% build.getLog(400).each() { line -> 
	...
	def (value1,value2,value3) = line.split(' ')
	def filenameConsole = "BuildConsole_"+value1+".txt"
	
	%>
		
		 <TR> <TD class="bg6"> Job Name: <B>${value1}</B></TD></TR>

		 <%
	
	if (build.getParent().getWorkspace().child(filenameConsole).exists()) {
		 build.getParent().getWorkspace().child(filenameConsole).readToString().eachLine(){ line1 ->
		
		
		def (x,y) = line1.split('=')
		def buildconsole1 = y+"consoleText"
		def buildconsole = y+"consoleText"	
		def lineNumber=0

		%>
		 <TR><TD class="bg7">${y}</TD></TR>

    	
		<%

           
		buildconsole.toURL().eachLine()  { currentLine ->
		
		
	 %>  
	 
	 <TR><TD class="bg7">${currentLine}</TD></TR>
 <%
}
 }
		}
