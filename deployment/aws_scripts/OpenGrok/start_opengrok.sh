...
sudo chmod g+rwx "${ogk_etc_dir}"
sudo chmod g+rwx "${ogk_data_dir}"
...
sudo chmod g+rwx "${ogk_master_etc_dir}"
sudo chmod g+rwx "${ogk_master_data_dir}"
...
echo 'Wait until you see the line: "(time) <sync-index> [INFO] Index trigger completed" and index completed in container (see logs in container)'
echo ""
...
echo ""
echo "(crontab -l ; echo \"12 0,3,6,9,12,15,18,21 * * * /bin/bash -c '/opengrok/scripts/sync-index.sh \\\"<GIT-USER>\\\" \\\"<GIT-TOKEN>\\\" \\\"/opengrok/src\\\" \\\"uft/uft.devops/master/repolist/opengrok_sync.txt\\\" opengrok_all > \\\"/opengrok/sync.log\\\" 2>&1'\") | crontab -"
echo ""
echo "(crontab -l ; echo \"47 2,5,8,11,14,17,20,23 * * * /bin/bash -c '/opengrok-master/scripts/sync-index.sh \\\"<GIT-USER>\\\" \\\"<GIT-TOKEN>\\\" \\\"/opengrok-master/src\\\" \\\"uft/uft.devops/master/repolist/opengrok_sync_master.txt\\\" opengrok_master > \\\"/opengrok-master/sync.log\\\" 2>&1'\") | crontab -"
echo ""