...
class CSAPreparationUninstallUFT : CSAPreparation {
    ...
    [Boolean]CheckUFTExist(
        ...
        Write-Host $ExpressionResult -ForegroundColor DarkBlue -BackgroundColor Gray -Separator "`n"
        $ApplicationDir="\\${CSAName}\C`$\Program Files (x86)\Micro Focus\UFT One\bin\UFT.exe"
    ...
    [Void]StopMsiexecProcess(
        ...
        $output = Invoke-Command -Credential $CSACredential -ComputerName $CSAName -ScriptBlock {
            Get-Process -Name "msiexec" | Stop-Process -Force 
        }
        Write-Host "CSAPreparationUninstallUFT::StopMsiexecProcess output = ${output}" -ForegroundColor Green -BackgroundColor Black
